...
unsigned int memtest(unsigned int start, unsigned int end);//?????D???????memtest_sub 
...
#define MAX_TASKS_LV	100     //????????????????100??????
#define MAX_TASKLEVELS	10      // ???10??????? 
struct TSS32 {//???????????????GDT?????????????? 
	int backlink, esp0, ss0, esp1, ss1, esp2, ss2, cr3; //?????????????? 
	int eip, eflags, eax, ecx, edx, ebx, esp, ebp, esi, edi;//32??????? 
	int es, cs, ss, ds, fs, gs;//16???????
...
struct TASK *task_now(void);////??????? ??????struct TASK ???????
...
int TestAndSet(int *target);//使用TestAndSet方法
void TASin(int No);
void TASout(int No);
